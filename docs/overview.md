...
**Milestones 1-6 done.** The npm-workspace toolchain, sessions/connection (join, live player list, host role, reconnect), the 3D room shell (a real Blender-built "cozy tabletop game room" with a round table, first-person WASD + mouse-look movement, wall/table collision), player avatars (other connected players render as placeholder capsules and move live), the tabletop map/drawing (the host swaps the table's background image, any player draws directly on the physical table surface, both live for everyone), and dice (any player spawns/rolls/removes a die, server-authoritative result, animated and visible to everyone live) all work end to end — see [roadmap.md](roadmap.md) for exactly what each milestone covers and [changelog.md](changelog.md) for how they were built. No soundboard/mute yet (Milestone 7+).

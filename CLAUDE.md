...
Status: **Milestones 1-9 done, plus a file-uploads extension on top of M5/M7** — toolchain, sessions/connection, the 3D room shell (real Blender-built room, first-person movement/collision), player avatars, the tabletop map/drawing (host-swappable background by URL or file upload, live collaborative drawing on the physical table), dice (server-authoritative spawn/roll/remove, animated and visible to everyone), soundboard/mute (host-triggered sounds — built-in tones or player-uploaded files — everyone hears, self- or host-mute visible to the group), room interactables (a light switch, a full-screen sit-down mode at the table with a pen/eraser drawing toolbar, a physical soundboard console), and host authority hardening (every host-gated action proven, server-side, to reject a non-host) all work end to end. Remaining: Milestone 10 (performance & polish). Known gap: M9's cross-browser pass (Edge/Firefox) wasn't performed — this environment only drives Chrome. See [docs/roadmap.md](docs/roadmap.md) for milestones and [docs/overview.md](docs/overview.md) for the full picture.

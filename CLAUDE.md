...
- from the follow-up batch: a synced TV, tables of eight, minis, secret dice, host controls, four more windows onto a living 3D night, and a Halloween toggle;
- gadgets from the chest (camera and pinboard, flashlight, walkies, calculator), one held at a time and used with R;
- from the latest batch ("The cozy room, lived in"): per-player sound control, weather and a winter theme, windows and curtains, seats on the sofa and chairs, a record player, tea and popcorn, a cat, books by the host, and mood presets;
- player profiles: an image each player shares (a character sheet, say) that only they and the current host see.

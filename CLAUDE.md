...
Status: **All ten milestones done** (M1–M10), plus the owner's follow-up change requests and two rounds of extras. That includes the cozy room with a working TV, a seated chair view, real dice, chat, pings, the whiteboard and the wall soundboard. `npm run test:e2e` covers Chromium/Firefox/WebKit (and Edge on demand), which closed M9's browser-pass gap. **Deployed** as one container to the owner's VPS (`npm run deploy`, [docs/engineering/deployment.md](docs/engineering/deployment.md)). It will be live at tabletop.murri.me once the owner adds the Nginx Proxy Manager host. Nothing on the roadmap remains. The "later" list is ideas in discussion; build none without the owner's go-ahead. See [docs/roadmap.md](docs/roadmap.md) for milestones and [docs/overview.md](docs/overview.md) for the full picture.

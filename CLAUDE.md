...
Status: **Milestones 1-6 done** — toolchain, sessions/connection, the 3D room shell (real Blender-built room, first-person movement/collision), player avatars, the tabletop map/drawing (host-swappable background, live collaborative drawing on the physical table), and dice (server-authoritative spawn/roll/remove, animated and visible to everyone) all work end to end. No soundboard/mute yet. See [docs/roadmap.md](docs/roadmap.md) for milestones and [docs/overview.md](docs/overview.md) for the full picture.
